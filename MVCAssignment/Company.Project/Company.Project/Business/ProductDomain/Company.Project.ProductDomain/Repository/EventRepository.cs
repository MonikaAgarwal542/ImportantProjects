   ...
   public class EventRepository : IEventRepository
    {
        ...
        {
            return _bookReadingContext.Events.OrderByDescending(x => x.Date).ThenBy(x => x.StartTime).ToList();
        }
        ...
        {
            using (_bookReadingContext)
            {

                var result = (from i in _bookReadingContext.Invitations
                              where i.EventId == evt.EventId
                              select i);

                _bookReadingContext.Invitations.RemoveRange(result.ToList());
                ...
                _bookReadingContext.Entry(evt).State = EntityState.Modified;
              //  _bookReadingContext.Events.Add(evt);
        ...
        public IEnumerable<Event> GetMyEvent(string userName)
        {
            var evt = (from e in _bookReadingContext.Events
                       ...
                       select e).OrderByDescending(x=>x.Date).ThenBy(x=>x.StartTime).ToList();
        ...
        public Event EditEventById(int eventId)
        {
            IEnumerable<Event> events = _bookReadingContext.Events;
            Event eventFound = null;
            var query = from evt in events
        ...
        public int CountComments(int eventId)
        {
            IEnumerable<Comment> comments = null;
        ...
        public void DeleteEventById(int eventId)
        {
           ...
           _bookReadingContext.Events.Remove(_bookReadingContext.Events.Find(eventId));
            _bookReadingContext.SaveChanges();
        }
